...
- [ ] Implement the first-slice DNS behavior in a way that matches the existing
      design contract for:
      - AAAA-only answers
      - A-only answers
      - dual-stack answers
      - NXDOMAIN / NODATA
- [ ] Define and persist authoritative mapping records under module-owned state
      rather than in transient process memory only
- [ ] Implement mapping refresh and expiry semantics that match the declared
      `mappingTtl` / `gcInterval` contract
- [ ] Render backend-facing mapping/config artifacts deterministically from that
      authoritative mapping state
- [ ] Keep the backend contract backend-agnostic in public semantics even if the
...
- [ ] A supported legacy IPv4-only client path can be explained end-to-end from
      DNS request to rendered translation artifact
- [ ] Mapping state survives process restarts in the declared persistent path
- [ ] Mapping expiry behavior is deterministic and testable
- [ ] DNS answer behavior matches the documented first-slice boundary
- [ ] The control-plane layer does not silently bypass the current explicit
