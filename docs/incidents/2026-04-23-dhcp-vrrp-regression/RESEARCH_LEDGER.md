...
| **H8** | **The remaining HA outage is caused by deployment mismatch between nodes, not another Kea code-path bug.** | **Retired** | E44 replaced the last mixed-URL picture; the current live pair is aligned in a different but still unsupported way: neither node is running Kea HA. |
| **H9** | **The live router pair has regressed or never completed the staged Kea HA rollout, and both nodes are currently running a non-HA DHCP4 config shape.** | **Resolved** | E44 established the live non-HA reality. E45 turned that from ambiguous drift into an explicit single-active/manual-promotion boundary for this pair. |
...
| E44 | Fresh live Kea control/config audit on `router` and `router-backup` | `router` is actively serving fresh DHCP traffic on `10.10.10.2`, but its live `/etc/static/kea/dhcp4-server.conf` contains no `hooks-libraries`, and `list-commands` on `/run/kea/dhcp4.sock` exposes no HA commands. `router-backup` now has LAN carrier on `ens19` and management on `ens18`, but `kea-dhcp4-server` is inactive because `ExecCondition=/bin/bash -c 'exit 1'`, there is no `/run/kea/dhcp4.sock`, and its live DHCP4 config has the same non-HA shape. | E43 is no longer the latest live state. The current boundary is not mixed HA URLs; it is that the live pair is not running Kea HA at all, so the incident cannot honestly be closed as HA convergence. |
| E45 | Real multi-seat planning round on router Kea HA direction | Codex CLI, Gemini CLI, DeepSeek API, and a real OpenCode enrichment seat all converged on the same practical answer: do not treat active Kea HA restoration as the default next move for this pair. Standardize the current non-HA/single-active DHCP model, frame `router-backup` as a manual promotion target, and reopen HA only as a fresh design effort with a higher proof bar. | This resolved the remaining ambiguity in H9. The incident no longer needs to stay open waiting for “HA convergence”; the support boundary is now explicit. |
